...
public class Polynomial extends ArrayList<Integer> {
    ...
    */
    ...
    public static int degreeCoeffsLowToHigh(int [] coeffsLowToHigh) {
        int size = coeffsLowToHigh.length;
        int degree = 0;
        for(int i=1; i<size; i++){
            if(coeffsLowToHigh[i]!=0 && i!=size-1){
                degree++;
            }
        }
    ...
    */
    ...
    public static int degreeCoeffsHighToLow(int [] coeffsHighToLow) {
      int size = coeffsHighToLow.length;
        int degree = 0;
        for(int i=size-1; i>=0; i--){
            if(coeffsHighToLow[i]!=0 && i!=size-1){
                degree++;
            }
        }
    ...
    */
    ...
    boolean lz=true;
    int lead = 0; 
    if(coeffsHighToLow.length==1 && coeffsHighToLow[0]==0){
        return coeffsHighToLow;
    }
    for(int i=0; i<coeffsHighToLow.length; i++){
        if(coeffsHighToLow[i]==0 && lz){
            lead++;
        }
        else if(coeffsHighToLow[i]!=0 && lz){
            lz = false;
        }
    }
    if(lead == coeffsHighToLow.length){
        return new int[]{0};
    }
	int [] highToLow = new int[coeffsHighToLow.length-lead];
    int x  = highToLow.length-1;
    for(int i=lead; i<coeffsHighToLow.length; i++){
       highToLow[x]=coeffsHighToLow[i];
       x--;
    }
	return highToLow;
    }
    ...
    */
    ...
    public static int [] highToLow(int [] coeffsLowToHigh) {
        boolean lz=true;
        int tail = 0;
        if(coeffsLowToHigh.length==1 && coeffsLowToHigh[0]==0){
            return coeffsLowToHigh;
        }
        for(int i=coeffsLowToHigh.length-1; i>=0; i--){
            if(coeffsLowToHigh[i]==0 && lz){
                tail++;
            }
            else if(coeffsLowToHigh[i]!=0 && lz){
                lz = false;
            }
        }
        if(tail == coeffsLowToHigh.length){
            return new int[]{0};
        }
        int [] highToLow = new int[coeffsLowToHigh.length-tail];
        int x  = 0;
        for(int i=coeffsLowToHigh.length-1-tail; i>=0; i--){
            highToLow[x]=coeffsLowToHigh[i];
               x++;
    }
    return highToLow;
    }
    private int indexMax(int [] coeffs) {
    ...
    private int [] invert(int [] coeffs) {
	
	int nonzeromax = indexMax(coeffs);
	
	int [] finalC = new int [nonzeromax+1];
	
	int j=0;
	for (int i=nonzeromax; i>= 0; i--) {
	    finalC[i] = coeffs[j];
	    j++;
	}
    ...
    */
    ...
    int curDegree = this.getDegree();
	int paramDegree = p.getDegree();

	int numCoeffs = 
	    (curDegree > paramDegree)? curDegree + 1 : paramDegree + 1;
	...
	int [] finalC = invert(coeffs);
	
	return new Polynomial (finalC);
    }
